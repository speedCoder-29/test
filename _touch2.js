...
html=html.replace('function endGame(impostorWins) {','function endGame(impostorWins) { if(window.__noWin)return;');
html=html.replace('function findSafeSpawn(x, y, w, h, avoid) {',
 'window.__gaps=function(){var o=[];for(var n of npcs){if(!n.alive)continue;var g=9;for(var m=0;m<=8;m++){if(isBlockedAt(n.x-m,n.y-m,n.w+2*m,n.h+2*m)){g=m;break;}}o.push(g);}return o;};'+
...
setTimeout(()=>{
  ...
  for(const mp of ['skeld','polus','airship','fungle','mira']){
    ...
    for(let c=0;c<120;c++){ flush(20); const g=w.__gaps(); g.forEach(v=>{hist[v]=(hist[v]||0)+1;samples++;if(v===0)touch++;if(v<3)close++;}); }
  }
  console.log('BASELINE  samples:',samples);
  console.log('  TOUCHING wall (gap=0):',(100*touch/samples).toFixed(2)+'%');
  console.log('  within <3px:',(100*close/samples).toFixed(2)+'%');
